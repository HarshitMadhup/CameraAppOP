...
class _GalleryState extends State<VideoGallery> {
  ...
  _fetchAssets() async {
    ...
    setState(() {
      for (int i = 0; i < assets.length; i++) {
        if (assets[i].type == AssetType.video) newassets.add(assets[i]);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GridView.builder(
        ...
        itemBuilder: (_, index) {
          return AssetThumbnail(asset: newassets[index]);
        },
      ),
    );
  }
